...
def get_seed(random_seed=''):
    ...
    try:
        ...
        return random_seed if random_seed < max_seed else random_seed - max_seed
    except:
...
def save_frame_timestamp(filename, timestamp='', random_seed=000, output_dir='output'):
    ...
    output_filename = os.path.join(
        output_dir, f'{timestamp}_{filename}_{random_seed}_####.png'
    )
...
def save_timestamp(filename, timestamp='', output_dir='output'):
    ...
    filename = filename.replace('\\', '')
    filename = filename.replace('/', '')
    output_filename = os.path.join(output_dir, f'{timestamp}_{filename}_####.png')
...
def random_centered(value_og, offset=5):
    ...
    return random(value_og-offset, value_og+offset)
